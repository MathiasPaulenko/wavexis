...
browsix is under active development. Security fixes are applied to the latest
release only.
...
If you discover a security vulnerability in browsix, please report it
responsibly:
...
browsix wraps cdpwave and bidiwave to control browsers via CDP and WebDriver BiDi.
Keep the following in mind:

- **Remote debugging port**: browsix uses `--remote-debugging-port` via cdpwave
  which exposes a WebSocket endpoint. In production, ensure this port is not
  accessible to untrusted networks.
- **Arbitrary JavaScript execution**: `browsix eval` executes arbitrary
...
- **URL navigation**: `browsix screenshot` and other commands can open any URL.
